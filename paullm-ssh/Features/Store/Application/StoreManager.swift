...
final class StoreManager: ObservableObject {
    ...
    var monthlyProduct: Product? {
        products.first { $0.id == paullm-sshProducts.proMonthly }
    }

    var yearlyProduct: Product? {
        products.first { $0.id == paullm-sshProducts.proYearly }
    }

    var lifetimeProduct: Product? {
        products.first { $0.id == paullm-sshProducts.proLifetime }
    }
    ...
    func loadProducts() async {
        ...
        for attempt in 0..<maxRetries {
            do {
                products = try await Product.products(for: paullm-sshProducts.allProducts)
    ...
    func checkEntitlements() async {
        ...
        for await result in Transaction.currentEntitlements {
            if case .verified(let transaction) = result {
                ...
                case paullm-sshProducts.proMonthly,
                     paullm-sshProducts.proYearly:
                    hasAccess = true
                case paullm-sshProducts.proLifetime:
                    hasAccess = true
                    hasLifetime = true
                default:
                    break
