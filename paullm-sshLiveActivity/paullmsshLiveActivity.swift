...
@available(iOS 16.1, *)
struct paullmsshLiveActivityWidget: Widget {
    var body: some WidgetConfiguration {
        ActivityConfiguration(for: paullmsshActivityAttributes.self) { context in
            paullmsshLiveActivityLockScreenView(context: context)
        } dynamicIsland: { context in
            DynamicIsland {
                DynamicIslandExpandedRegion(.leading) {
                    HStack(spacing: 8) {
                        AppIconView(size: 20)
                        Text(String(localized: "paullmssh"))
                            .font(.headline)
...
@available(iOS 16.1, *)
private struct paullmsshLiveActivityLockScreenView: View {
    let context: ActivityViewContext<paullmsshActivityAttributes>
